...
def the_sequence_optimizer(ancestor_sequence, codon_use_table, minimum_fitness, generations, gc_content, restriction_sites, rbs, num_individuals=25):
    ...
    def _generate_initial_population(pop_size=10):
        """Create the initial population of mutated sequences"""
        mutants = population
        mutable_seq = ancestor_sequence.tomutable()
        for idx in range(pop_size):
            new_seq = mutable_seq
            new_seq = new_seq.toseq()
            mutants[mutate_sequence(new_seq)] = sys.maxsize
        return mutants

    def mutate_sequence(sequenc, offset=0, mutation_probability=0.05):
        """
    ...
    def _select_best_offspring():
        ...
        if num_parents < 1:
            num = 1
        return dict(sorted(population.items(), key=operator.itemgetter(1), reverse=False)[:num_parents])

    def _iterate_population(best, population_size=10):
        # first remove non best performers by setting population as best
        new_pop = best

        # add new mutants while keeping the top 20%
        while len(new_pop) < population_size:
            new_seq = mutate_sequence(random.choice(list(best.keys())))
            new_pop[new_seq] = sys.maxsize
        return new_pop
    ...
    def _eval_host():
        """
        Evaluates the sequence based on the number of differences 
        with the optimal expression sequence given to the algorithm
